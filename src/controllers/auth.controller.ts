...
class AuthController {
    ...
    private initRoutes() {
        ...
        this.router.post("/verify-token", this.validateBody("verify-token"), this.verifyToken)
        this.router.post("/forgot", this.validateBody("forgot"), this.forgotPasswordCode)
        this.router.post("/confirm-forgot", this.validateBody("confirmForgot"), this.confirmForgotPassword)
    ...
    signup(req: Request, res: Response) {
        ...
        let userAttributes = []
        Object.keys(attributes).forEach((key: string) => userAttributes.push({ Name: key, Value: attributes[key] }))
    ...
    confirmForgotPassword(req: Request, res: Response) {
        ...
        const cognito = new CognitoService()
        cognito.changePassword(username, confirmationCode, password).then(success => {
            if (success) res.status(200).send(true)
            else res.status(401).send(false)
        })
    }
